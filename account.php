...
<title>NITK TEST PORTAL || ACCOUNT </title>
...
<nav class="navbar navbar-default title1">
  <div class="container-fluid">
    ...
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="#"><b>N.I.T.K</b></a>
    </div>
...
$sn=@$_GET['n'];
if($sn==1){
  $temp=mysqli_query($con,"SELECT * FROM questions WHERE eid='$eid'");
  $len=mysqli_num_rows($temp);
  $arr=array();
  for($x=1;$x<=$len;$x++){
    array_push($arr,$x);
  }
  shuffle($arr);
  $yup=mysqli_query($con,"SELECT * FROM questions WHERE eid='$eid'" );
  $x=0;
  while($row=mysqli_fetch_array($yup) )
  {
    $popl=$row['qid'];
    mysqli_query($con,"UPDATE questions SET sn='$arr[$x]' WHERE qid='$popl'");
    $x++;
  }
...
$q=mysqli_query($con,"SELECT * FROM options WHERE options.qid='$qid' " );
...
<a href="http://www.nitk.ac.in" target="_blank">About us</a>
</div>
<div class="col-md-3 box">
...
<div class="modal fade title1" id="developers">
  <div class="modal-dialog">
    ...
    <div class="row">
    <div class="col-md-4">
     <img src="image/nitk1.png" width=100 height=100 alt="" class="img-rounded">
     </div>
     <div class="col-md-5">
    <h4 style="color:#202020; font-family:'typo' ; font-size:18px">Akshay Pandita</h4></div></div>
    </p>
      </div>
